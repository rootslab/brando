...
var me = this,
            ...
            offset = 0,
            // max offset
            limit = data.length - bytes,
            num = 0,
            feed = false,
            check = me.check;

        for ( ; me.k < n ; offset += bytes ) {
            if ( offset > limit ) { feed = true; break; }
            num = me.readUInt( data, offset, me.shift, true );
            if ( me._check( num ) ) {
                me.writeUInt( cfg.output, num, me.k, true );
                me.k += bytes;
            } else {
                // current value was excluded
                ++me.x;
            }
        };
        if ( feed ) {
