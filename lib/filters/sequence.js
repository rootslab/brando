...
module.exports = ( function () {
    ...
    sproto.parse = function ( data ) {
         ...
         *
         * NOTE: when the selected range is within the next power of 2, the probability to discard an
         * element, parsed from the random source, decreases; then, it's better to copy an entire
         * slice of values, instead of a value at a time (#sparse method).
         *
         * Example :
         * - 2^(4) or 32 is an optimal choose for a range (values from 0 to 31), after cutting bits from
         *   every values parsed, we expect to accept the value with probability 1.
         * - 2^(4) + 1 or 33 is the worst value, because it needs 2 bytes of random data, but it doesn't
         *   accept values from 32 to 2^(16) or 65535, then, after cutting bits (7) we have a probability
         *   <1/2 of discarding a parsed value from random source.
         */
